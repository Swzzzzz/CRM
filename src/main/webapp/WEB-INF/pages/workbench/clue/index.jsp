...
<script type="text/javascript">
	...
	$(function(){
		...
		$("#saveClueBtn").click(function () {
			...
			var yxStr = /^\w+([-+.]\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$/;
			var gszjStr = /\d{3}-\d{8}|\d{4}-\d{7}/;
			var sjStr = /^(13[0-9]|14[5|7]|15[0|1|2|3|5|6|7|8|9]|18[0|1|2|3|5|6|7|8|9])\d{8}$/;
			//表单验证
			if (company==""){
				alert("公司名称不能为空");
				return;
				...
				alert("姓名不能为空");
				return;
			}else if(!yxStr.test(email) && email==""){
				alert("邮箱不合法")
				return;
			}else if (!gszjStr.test(phone) && phone==""){
				alert("手机号码不合法");
				return;
			}else if(!sjStr.test(mphone) && mphone==""){
				alert("座机格式不合法");
				return;
			}
								...
								<select class="form-control" id="create-state">
								  <option></option>
									<c:forEach items="${clueStateList}" var="clue">
										<option id="${clue.id}">${clue.value}</option>
									</c:forEach>
								  <%--<option>试图联系</option>
								  <option>将来联系</option>
								  <option>已联系</option>
			...
			<div style="position: relative;top: 50px;">
				<table class="table table-hover">
					...
					<tbody>
						<tr>
							<td><input type="checkbox" /></td>
							<td><a style="text-decoration: none; cursor: pointer;" onclick="window.location.href='workbench/clue/detailClue.do?id=83a5fe800c2e49a891ebfcb94b03afe6';">张三博士</a></td>
							<td>动力节点</td>
							<td>010-84846003</td>
							<td>12345678901</td>
							<td>广告</td>
